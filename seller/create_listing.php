<?php  
   ...
   $agentQuery = "  
      SELECT Agent.agentID, Users.username, Agent.areaInCharge  
      FROM Agent  
   ...
   <select id="agentID" name="agentID" class="form-control" required>  
      ...
      if ($agentResult->num_rows > 0) {  
        while ($row = $agentResult->fetch_assoc()) {  
           ...
           $areaInCharge = htmlspecialchars($row['areaInCharge']);  
           echo "<option value=\"" . $agentID . "\" data-area-in-charge=\"" . $areaInCharge . "\">" . $agentID . " - " . $agentName . "</option>";  
        }  
      } else {  
